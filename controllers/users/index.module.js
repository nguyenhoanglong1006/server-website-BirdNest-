module.exports = (req) => {
	...
	return new function () {
		...
		_this.getRow = async (id) => {
			...
			let sql = "SELECT * FROM " + table.users + " WHERE id =  $1 ";
		...
		_this.changePassword = async (password, token, id) => {
			...
			let sql = "UPDATE " + table.users + " SET password = $1 , token = $2 WHERE id = $3 ";
		...
		_this.checkProcessEmail = async (email, id) => {
			...
			let sql = "SELECT COUNT(id) AS count FROM " + table.users + " WHERE email = $1 ";
		...
		_this.process = async (data, id) => {
			...
			let sql = (id == 0) ? "INSERT INTO " + table.users + " SET $" : "UPDATE " + table.users + " SET $ WHERE id= $ ";
		...
		_this.remove = async (id) => {
			...
			let sql = " DELETE FROM " + table.users + " WHERE is_delete != 1 AND id = $1 ";
